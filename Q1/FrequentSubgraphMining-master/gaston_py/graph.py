...
def draw_nx_graphs(output_file_path, frequent_output):
    ...
    for graph_id, (embedding, (graph, graph_type, frequency)) in enumerate(frequent_output.items()):
        ...
        node_labels = {id: data['label'] for (id, data) in list(graph.nodes(data=True))}
        edge_labels = {(u, v): data['label'] for (u, v, data) in list(graph.edges(data=True))}
...
def write_line_graphs(graphs, file_path):
    ...
    with open(file_path, "w") as f:
        for g_id, graph in enumerate(graphs):
            ...
            for index, (node_id, data) in enumerate(list(graph.nodes(data=True))):
...
def count_unique_nodes(graphs):
    return len(set(data['label'] for graph in graphs for _, data in list(graph.nodes(data=True))))

def count_unique_edges(graphs):
    return len(set(edge['label'] for graph in graphs for _, _, edge in list(graph.edges(data=True))))
